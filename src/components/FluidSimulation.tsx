...
const particleRenderFS = `
    ...
    void main() {
        ...
        float alpha = 1.0 - v_age_ratio;
        gl_FragColor = vec4(uParticleColor, alpha * 0.5);
    }
`;
...
const surfaceDetailShader = `
    ...
    float sample_noise(vec2 uv, float flow_mag) {
        // Animate faster when flow is stronger
        float time_mult = 1.0 + flow_mag * 5.0;
        float n1 = snoise(uv * 40.0 + uTime * 0.8 * time_mult);
        float n2 = snoise(uv * 90.0 - uTime * 0.5 * time_mult) * 0.5;
        return n1 + n2;
    }
    ...
    void main() {
        ...
        vec2 sample_uv = vUv + flow;
        float noise_val = sample_noise(sample_uv, flow_mag);

        vec2 dUv = uTexelSize * 2.0;
        float nx = sample_noise(sample_uv + vec2(dUv.x, 0.0), flow_mag);
        float ny = sample_noise(sample_uv + vec2(0.0, dUv.y), flow_mag);
...
const FluidEffect: React.FC<FluidEffectProps> = ({ params }) => {
...
const ControlsPanel: React.FC<ControlsPanelProps> = ({ params, setParams, activePreset, selectPreset }) => {
